	...
	for( i=0; i < polygons->lastcol-2; i+=3 ) {
		...
		if ( calculate_dot( polygons, i ) < 0 ) {
			...
			if (strcmp(shading, "flat") == 0){
				...
				col.red = get_illumination(l -> c, normal, l -> l[0], c -> r);
				col.green = get_illumination(l -> c, normal, l -> l[1], c -> g);
				col.blue = get_illumination(l -> c, normal, l -> l[2], c -> b);
			}
	 
			fill(polygons, i, s, col, zb, vns, l, c, shading);
